...
const WomensCollection = () => {
  ...
  const [category, setCategory] = useState("Women"); // Hardcoded category name
  ...
  const getProductsForWomenCategory = async () => {
    try {
      const { data } = await axios.get(
        "/api/v1/product/product-category/women" // Hardcoded category slug
      );
  ...
  return (
    <Layout>
      <div className="container mt-3 category">
        <h4 className="text-center"><b>Women's Collection</b></h4>
